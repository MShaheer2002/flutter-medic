...
**Root cause found** (unlike the Patrol investigation — this one resolved). Read `dart_mcp_server`'s actual source (`_AppListener` in `lib/src/mixins/dtd.dart`, package cached locally at `~/.pub-cache/hosted/pub.dev/dart_mcp_server-1.1.1`). `get_runtime_errors` is backed by exactly two sources: VM service extension events where `extensionKind == 'Flutter.Error'` (what `FlutterError.onError` publishes — framework-level build/layout/paint errors, and zone-caught errors), and the VM service's `Stderr` event stream (mirrors the isolate's `dart:io` stderr writes).

**Neither source ever receives this specific error class.** The captured log line — `[ERROR:flutter/runtime/dart_vm_initializer.cc(40)] Unhandled Exception: ...` — is the Dart VM's raw C++ fallback handler, the true last resort when nothing in Dart-land catches an error at all (no zone, no `FlutterError.onError`). It prints straight through the platform's native logging (Android's `__android_log_print`, producing the `E/flutter` logcat tag) — bypassing `dart:io` stderr and never firing a `Flutter.Error` extension event.

`home_screen.dart`'s `_loadTasks()` is called fire-and-forget from `initState()` (`_loadTasks();`, no `await`, no `.catchError()`) — an exception thrown inside it is an unhandled `Future` rejection with no zone wrapping it, so it falls straight through both channels `get_runtime_errors` listens to, all the way to the VM's raw fallback.

**This is a structural gap, not an integration bug**: this specific error class (unhandled fire-and-forget async exception, no zone handler) is invisible to `Extension.Flutter.Error` and VM-service-`Stderr` by construction — no amount of connection-timing or retry logic fixes it. The only place it's genuinely visible is native platform logging (`adb logcat`, filtered to the app's process) — a *third* log-capture path beyond what the spec's §11 already anticipated (`print`/`debugPrint` → stdout, `dart:developer log()` → VM service stream). Kept the `runtime-exception` rule in the code regardless — it correctly reports "clean" for genuinely clean runs, and still catches the more common framework-level/zone-caught error classes; this is a documented gap in coverage, not a broken feature.
