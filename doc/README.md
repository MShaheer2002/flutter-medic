...
**Phase 1, checked against the spec's own definition (§Roadmap, §12)**: orchestration MCP server ✅, tool routing ✅, device detection ✅, launch/interact/observe ✅. Console log capture is real but has one understood, unfixed gap (below). Natural-language goal parsing is explicitly not attempted — that's Phase 2.

**Known, understood gap**: the `runtime-exception` tier-1 rule structurally can't catch unhandled fire-and-forget async exceptions — root cause found (see 007): `get_runtime_errors` only listens to `Flutter.Error` extension events and the VM service's `Stderr` stream, and this error class (no zone handler at all) bypasses both, falling straight to the Dart VM's raw native-log fallback instead. Not fixable by retrying or reconnecting differently — the only place this error class is visible at all is native platform logging (`adb logcat`), a third log-capture path beyond the two the spec's §11 anticipated. The `expected-element-missing` rule is solid, proven against both the real silent bug and a real injected exception.

**Next up (not yet decided)**: add a logcat-based detection path to close the gap above, add tier-2 (baseline diff) or tier-3 (handing raw evidence to an LLM for judgment) anomaly detection, start on `flutter-medic init` (§14), or stress-test how general the current parameterization really is against a second, different demo app.
